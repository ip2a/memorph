...
pub struct QoderHookAdapter;

impl HookAdapter for QoderHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("qoder", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = QoderHookAdapter.normalize(&request).unwrap().remove(0);
