//! Trae / TraeCli hook adapter.
//!
//! TraeCli uses snake_case hook names in `~/.trae/traecli.yaml`. This adapter
//! maps those events into memorph's canonical runtime model while keeping the
//! provider-specific payload parsing isolated from the hook runtime.
...
pub struct TraeHookAdapter;

impl HookAdapter for TraeHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("trae", trae_event_type(&event_name), raw.clone());
...
fn trae_event_type(event_name: &str) -> HookEventType {
    match normalize_name(event_name).as_str() {
        ...
        "askuserquestion" | "questionrequest" => HookEventType::QuestionRequested,
        "subagentstart" | "subagentstop" | "precompact" | "postcompact" => HookEventType::Heartbeat,
...
mod tests {
    ...
    #[test]
    fn maps_traecli_permission_request() {
        ...
        let event = TraeHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "trae");
        assert_eq!(event.event_type, HookEventType::PermissionRequested);
        assert_eq!(event.provider_session_id.as_deref(), Some("s1"));
        assert_eq!(event.permission.unwrap().request_id.as_deref(), Some("r1"));
    }
}
