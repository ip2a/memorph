...
pub struct TraeCnHookAdapter;

impl HookAdapter for TraeCnHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("traecn", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = TraeCnHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "traecn");
