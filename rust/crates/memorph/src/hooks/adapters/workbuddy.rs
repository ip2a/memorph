...
pub struct WorkBuddyHookAdapter;

impl HookAdapter for WorkBuddyHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("workbuddy", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = WorkBuddyHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "workbuddy");
