...
pub struct StepFunHookAdapter;

impl HookAdapter for StepFunHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("stepfun", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = StepFunHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "stepfun");
