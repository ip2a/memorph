//! Codex hook adapter.
//!
//! Codex hook payloads are JSON and usually use the same lifecycle event names
//! as the hook configuration file. This adapter keeps the Codex-specific alias
//! handling separate from the generic adapter so hook health can be promoted to
//! a first-class provider without changing runtime state code.
...
impl HookAdapter for CodexHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("codex", codex_event_type(&event_name), raw.clone());
...
fn codex_event_type(event_name: &str) -> HookEventType {
...
mod tests {
    ...
    fn maps_codex_pre_tool_use() {
        ...
        let event = CodexHookAdapter.normalize(&request).unwrap().remove(0);
        ...
        assert_eq!(event.tool.as_ref().unwrap().id.as_deref(), Some("call-1"));
    }

    #[test]
    fn maps_codex_permission_request() {
        let request = HookIngestRequest::new(
            "codex",
            "PermissionRequest",
            json!({
                ...
                "message": "Allow shell escalation?"
            }),
        );
        let event = CodexHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.event_type, HookEventType::PermissionRequested);
        assert_eq!(
            event.permission.as_ref().unwrap().request_id.as_deref(),
            Some("call-2")
        );
        assert_eq!(
            event.permission.as_ref().unwrap().prompt.as_deref(),
            Some("Allow shell escalation?")
        );
    }

    #[test]
    fn falls_back_to_bridge_environment() {
        let mut request = HookIngestRequest::new("codex", "SessionStart", json!({}));
    ...
    fn maps_user_prompt_submit_to_user_message() {
        let request = HookIngestRequest::new(
            "codex",
            "UserPromptSubmit",
            json!({"user_prompt": "explain this diff"}),
        );
        let event = CodexHookAdapter.normalize(&request).unwrap().remove(0);
        ...
        assert_eq!(event.message.as_ref().unwrap().text, "explain this diff");
    }
    ...
    fn maps_stop_to_assistant_message() {
        let request = HookIngestRequest::new(
            "codex",
            "Stop",
            json!({"lastAssistantMessage": "tests passed"}),
        );
        let event = CodexHookAdapter.normalize(&request).unwrap().remove(0);
        ...
        assert_eq!(event.message.as_ref().unwrap().text, "tests passed");
    }
    ...
    fn uses_first_workspace_root_as_cwd() {
        let request = HookIngestRequest::new(
            "codex",
            "SessionStart",
            json!({"workspaceRoots": ["/tmp/codex-a", "/tmp/codex-b"]}),
        );
        let event = CodexHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(
            event.cwd.as_deref(),
            Some(std::path::Path::new("/tmp/codex-a"))
        );
    }
}
