...
pub struct TraeGuiHookAdapter;

impl HookAdapter for TraeGuiHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("trae_gui", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = TraeGuiHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "trae_gui");
