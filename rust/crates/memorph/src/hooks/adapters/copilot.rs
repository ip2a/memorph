...
//!
//! Copilot hook entries use `{ type, bash, timeoutSec }` and event names are
//! lower-camel. The bridge passes the event name explicitly because Copilot's
//! stdin payload may not include a stable hook event field.
...
impl HookAdapter for CopilotHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("copilot", copilot_event_type(&event_name), raw.clone());
...
fn copilot_event_type(event_name: &str) -> HookEventType {
    match normalize_name(event_name).as_str() {
        ...
        "userpromptsubmitted" | "userpromptsubmit" => HookEventType::MessageCreated,
        "pretooluse" => HookEventType::ToolStarted,
        "posttooluse" => HookEventType::ToolFinished,
        "erroroccurred" => HookEventType::SessionFailed,
...
fn message_at(event_name: &str, value: &Value) -> Option<HookMessage> {
    ...
    let (role, keys): (Option<String>, &[&str]) = match normalized.as_str() {
        ...
        "notification" | "erroroccurred" => {
            (None, &["message", "text", "summary", "status", "detail"])
        }
...
mod tests {
    ...
    fn maps_copilot_pre_tool_use() {
        ...
        let event = CopilotHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "copilot");
    ...
    #[test]
    fn maps_copilot_error_to_session_failed() {
        let request = HookIngestRequest::new(
            "copilot",
            "errorOccurred",
            json!({"session_id": "s1", "message": "boom"}),
        );
        let event = CopilotHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.event_type, HookEventType::SessionFailed);
        assert_eq!(event.message.as_ref().unwrap().text, "boom");
    }

    #[test]
    fn maps_copilot_user_prompt_to_user_message() {
        let request = HookIngestRequest::new(
            "copilot",
            "userPromptSubmitted",
        ...
        assert_eq!(event.message.as_ref().unwrap().text, "review this change");
    }

    #[test]
    fn maps_copilot_stop_to_assistant_message() {
        let request =
            HookIngestRequest::new("copilot", "sessionEnd", json!({"summary": "all done"}));
        let event = CopilotHookAdapter.normalize(&request).unwrap().remove(0);
        ...
        assert_eq!(event.message.as_ref().unwrap().text, "all done");
    }
    ...
    fn uses_first_workspace_root_as_cwd() {
        let request = HookIngestRequest::new(
            "copilot",
            "sessionStart",
            json!({"workspace_roots": ["/tmp/copilot-a", "/tmp/copilot-b"]}),
        );
        let event = CopilotHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(
            event.cwd.as_deref(),
            Some(std::path::Path::new("/tmp/copilot-a"))
        );
    }
}
