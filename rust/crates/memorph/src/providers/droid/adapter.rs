...
pub struct DroidHookAdapter;

impl HookAdapter for DroidHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("droid", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = DroidHookAdapter.normalize(&request).unwrap().remove(0);
