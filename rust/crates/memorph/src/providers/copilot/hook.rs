...
pub static COPILOT_HOOK: CopilotHook = CopilotHook;
...
const EVENTS: &[CopilotHookEvent] = &[
    event("sessionStart", 5, false),
    event("sessionEnd", 5, true),
    event("userPromptSubmitted", 5, false),
    event("preToolUse", 5, false),
    event("postToolUse", 5, true),
    event("errorOccurred", 5, true),
];

const fn event(name: &'static str, timeout_sec: u64, blocking: bool) -> CopilotHookEvent {
    CopilotHookEvent {
        name,
        timeout_sec,
        blocking,
...
impl ProviderHook for CopilotHook {
...
fn status() -> Result<HookInstallStatus> {
    ...
    if !path.exists() {
        return Ok(HookInstallStatus {
            ...
            last_event_at: crate::hooks::health::last_event_at("copilot"),
        });
    }
...
mod tests {
    ...
    fn descriptor_matches_hook_registry() {
        let descriptor = COPILOT_HOOK.descriptor().expect("copilot descriptor");
        assert_eq!(descriptor.provider(), COPILOT_HOOK.provider_id());
    }
}
