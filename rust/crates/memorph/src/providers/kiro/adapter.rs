//! Kiro hook adapter.
//!
//! Kiro uses an agent-scoped JSON hook file under `~/.kiro/agents/` and emits
//! camelCase hook events. This adapter maps those events into memorph's
//! canonical runtime state without changing the runtime reducer.
...
pub struct KiroHookAdapter;

impl HookAdapter for KiroHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("kiro", kiro_event_type(&event_name), raw.clone());
...
fn kiro_event_type(event_name: &str) -> HookEventType {
    match normalize_name(event_name).as_str() {
        "agentspawn" | "sessionstart" => HookEventType::SessionStarted,
        "userpromptsubmit" | "userpromptsubmitted" | "notification" => {
...
mod tests {
    ...
    #[test]
    fn maps_kiro_agent_spawn_to_session_started() {
        let request = HookIngestRequest::new(
            "kiro",
            "agentSpawn",
            json!({"session_id": "kiro-session-1", "cwd": "/tmp/project"}),
        );
        let event = KiroHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "kiro");
        assert_eq!(event.event_type, HookEventType::SessionStarted);
        assert_eq!(event.provider_session_id.as_deref(), Some("kiro-session-1"));
    }

    #[test]
    fn maps_kiro_pre_tool_use() {
        let request = HookIngestRequest::new(
            "kiro",
            "preToolUse",
            json!({
