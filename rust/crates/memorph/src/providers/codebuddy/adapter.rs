...
pub struct CodeBuddyHookAdapter;

impl HookAdapter for CodeBuddyHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("codebuddy", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = CodeBuddyHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "codebuddy");
