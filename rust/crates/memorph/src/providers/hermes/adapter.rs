...
pub struct HermesHookAdapter;

impl HookAdapter for HermesHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("hermes", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = HermesHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "hermes");
