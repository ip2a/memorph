...
pub static GEMINI_HOOK: GeminiHook = GeminiHook;
...
const EVENTS: &[GeminiHookEvent] = &[
    event("SessionStart", 10000, false),
    event("SessionEnd", 10000, false),
    event("BeforeTool", 10000, false),
    event("AfterTool", 10000, false),
    event("BeforeAgent", 10000, false),
    event("AfterAgent", 10000, false),
];

const fn event(name: &'static str, timeout: u64, blocking: bool) -> GeminiHookEvent {
    GeminiHookEvent {
        name,
        timeout,
        blocking,
...
impl ProviderHook for GeminiHook {
...
fn status() -> Result<HookInstallStatus> {
    ...
    if !path.exists() {
        return Ok(HookInstallStatus {
            ...
            last_event_at: crate::hooks::health::last_event_at("gemini"),
        });
    }
...
mod tests {
    ...
    fn descriptor_matches_hook_registry() {
        let descriptor = GEMINI_HOOK.descriptor().expect("gemini descriptor");
        assert_eq!(descriptor.provider(), GEMINI_HOOK.provider_id());
    }
}
