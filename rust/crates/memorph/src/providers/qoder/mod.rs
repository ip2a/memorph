...
impl Provider for QoderProvider {
    ...
    fn import_session(&self, source_path: &str) -> Result<ImportedSession> {
        let path = std::fs::canonicalize(source_path)
            .with_context(|| format!("Qoder source does not exist: {source_path}"))?;
        let id = session_id(&path)
            .context("Qoder source must be .qoder/projects/<project>/<session>.jsonl")?;
        ...
        extensions.insert("qoder_session_jsonl".into(), Value::Array(raw.clone()));
        let created = raw
            .iter()
            .find_map(timestamp)
    ...
    std::fs::read_to_string(path)?
        ...
        .map(|l| {
            serde_json::from_str(l)
                .with_context(|| format!("invalid Qoder event in {}", path.display()))
        })
        .collect()
}
...
fn map_event(v: &Value, i: usize, r: &mut MappingReport) -> Option<Event> {
    ...
    r.push_issue(crate::session::MappingIssue {
        ...
        code: "qoder-native-event".into(),
        message: "Mapped Factory/Qoder JSONL event".into(),
        path: Some(format!("events[{i}]")),
        raw: None,
    });
    Some(Event {
        id: format!("qoder:event:{i}"),
        kind,
        role,
...
mod tests {
    ...
    fn identity_comes_from_transcript_filename() {
        assert_eq!(
            session_id(Path::new("/tmp/.qoder/projects/project/abc.jsonl")).as_deref(),
            Some("abc")
        );
    }
    #[test]
