...
pub struct TraeGuiHook;

pub static TRAE_GUI_HOOK: TraeGuiHook = TraeGuiHook;

impl ProviderHook for TraeGuiHook {
...
fn status() -> Result<HookInstallStatus> {
    ...
    if !path.exists() {
        return Ok(HookInstallStatus {
            ...
            last_event_at: crate::hooks::health::last_event_at("trae_gui"),
        });
    }
...
mod tests {
    ...
    fn descriptor_matches_hook_registry() {
        let descriptor = TRAE_GUI_HOOK.descriptor().expect("trae gui descriptor");
        assert_eq!(descriptor.provider(), TRAE_GUI_HOOK.provider_id());
    }
    #[test]
    fn flat_json_install_uninstall_preserves_foreign_hooks() {
        ...
        TRAE_GUI_HOOK.uninstall().unwrap();
        let removed =
            crate::hooks::config_formats::json_hooks::read_json_object_or_empty(&path).unwrap();
    ...
    #[test]
    fn installs_and_uninstalls_trae_gui_flat_hooks() {
        let _home = TestHookHomeGuard::new();
        assert_eq!(
            TRAE_GUI_HOOK.verify().unwrap().status.status,
            HookHealthStatus::NotInstalled
        );
        let installed = TRAE_GUI_HOOK.install().unwrap();
        ...
        assert!(contents.contains("--provider trae_gui"));

        let removed = TRAE_GUI_HOOK.uninstall().unwrap();
