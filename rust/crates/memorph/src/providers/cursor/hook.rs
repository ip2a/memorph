...
pub static CURSOR_HOOK: CursorHook = CursorHook;
...
const fn event(name: &'static str, blocking: bool) -> CursorHookEvent {
    CursorHookEvent { name, blocking }
}

impl ProviderHook for CursorHook {
...
fn status() -> Result<HookInstallStatus> {
    ...
    if !path.exists() {
        return Ok(HookInstallStatus {
            ...
            last_event_at: crate::hooks::health::last_event_at("cursor"),
        });
    }
...
mod tests {
    ...
    fn descriptor_matches_hook_registry() {
        let descriptor = CURSOR_HOOK.descriptor().expect("cursor descriptor");
        assert_eq!(descriptor.provider(), CURSOR_HOOK.provider_id());
    }
}
