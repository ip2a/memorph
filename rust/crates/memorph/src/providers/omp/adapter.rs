...
pub struct OmpHookAdapter;

impl HookAdapter for OmpHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("omp", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = OmpHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(event.provider, "omp");
