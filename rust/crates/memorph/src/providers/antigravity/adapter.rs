...
pub struct AntiGravityHookAdapter;

impl HookAdapter for AntiGravityHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event =
            HookEvent::new("antigravity", provider_event_type(&event_name), raw.clone());
...
mod tests {
    ...
    fn maps_pre_tool_use() {
        ...
        let event = AntiGravityHookAdapter
            .normalize(&request)
            .unwrap()
            .remove(0);
