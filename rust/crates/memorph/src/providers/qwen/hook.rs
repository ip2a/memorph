...
pub static QWEN_HOOK: QwenHook = QwenHook;
...
const EVENTS: &[QwenHookEvent] = &[
    event("UserPromptSubmit", 5000, true),
    event("PreToolUse", 5000, false),
    event("PostToolUse", 5000, true),
    event("PostToolUseFailure", 5000, true),
    event("PermissionRequest", 86400000, false),
    event("Stop", 5000, true),
    event("SubagentStart", 5000, true),
    event("SubagentStop", 5000, true),
    event("SessionStart", 5000, false),
    event("SessionEnd", 5000, true),
    event("Notification", 86400000, false),
    event("PreCompact", 5000, true),
];

const fn event(name: &'static str, timeout: u64, blocking: bool) -> QwenHookEvent {
    QwenHookEvent {
        name,
        timeout,
        blocking,
...
impl ProviderHook for QwenHook {
...
fn status() -> Result<HookInstallStatus> {
    ...
    if !path.exists() {
        return Ok(HookInstallStatus {
            ...
            last_event_at: crate::hooks::health::last_event_at("qwen"),
        });
    }
...
mod tests {
    ...
    fn descriptor_matches_hook_registry() {
        let descriptor = QWEN_HOOK.descriptor().expect("qwen descriptor");
        assert_eq!(descriptor.provider(), QWEN_HOOK.provider_id());
    }
}
