//! Qwen hook adapter.
//!
//! Qwen Code uses Claude-like JSON hook configuration with millisecond
//! timeouts. This adapter keeps Qwen payload aliases isolated from the generic
...
pub struct QwenHookAdapter;

impl HookAdapter for QwenHookAdapter {
    ...
    fn normalize(&self, request: &HookIngestRequest) -> Result<Vec<HookEvent>> {
        ...
        let mut event = HookEvent::new("qwen", qwen_event_type(&event_name), raw.clone());
...
fn qwen_event_type(event_name: &str) -> HookEventType {
...
mod tests {
    ...
    #[test]
    fn maps_qwen_stop_to_assistant_message() {
        let request =
            HookIngestRequest::new("qwen", "Stop", json!({"lastAssistantMessage": "finished"}));
        let event = QwenHookAdapter.normalize(&request).unwrap().remove(0);
        ...
        assert_eq!(event.message.as_ref().unwrap().text, "finished");
    }
    ...
    fn uses_first_workspace_root_as_cwd() {
        let request = HookIngestRequest::new(
            "qwen",
            "SessionStart",
            json!({"workspace_roots": ["/tmp/qwen-a", "/tmp/qwen-b"]}),
        );
        let event = QwenHookAdapter.normalize(&request).unwrap().remove(0);
        assert_eq!(
            event.cwd.as_deref(),
            Some(std::path::Path::new("/tmp/qwen-a"))
        );
    }
}
