  ...
  return (
    <div className="flex flex-col gap-4" data-codex-workspace-repair-result>
      <MetricGrid columns="auto">
        <MetricTile label="Repaired" value={report.repaired_session_count} hint="sessions synced" variant="compact" />
        <MetricTile label="Hidden" value={report.hidden_session_count} hint="before repair" variant="compact" />
        <MetricTile label="Reindexed" value={report.reindexed_session_count} variant="compact" />
        <MetricTile label="Retitled" value={report.retitled_session_count ?? 0} variant="compact" />
      </MetricGrid>
      ...
      <div className="grid gap-2 rounded-md border p-3 text-sm">
        ...
        <div className="grid gap-1 md:grid-cols-[minmax(140px,auto)_minmax(0,1fr)] md:items-center">
          <span className="text-muted-foreground font-mono text-xs uppercase">Provider</span>
        ...
        <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs">
          <span>Scanned {report.scanned_rollouts} rollouts</span>
          <span>{report.workspace_session_count} workspace sessions</span>
          <span>{report.sqlite_rows_updated} sqlite rows updated</span>
          {report.pruned_backup_count > 0 ? <span>{report.pruned_backup_count} backups pruned</span> : null}
        </div>
        {report.backup_dir ? (
          <div className="grid gap-1 md:grid-cols-[minmax(140px,auto)_minmax(0,1fr)] md:items-start">
            <span className="text-muted-foreground font-mono text-xs uppercase">Backup</span>
      ...
      <div className="flex flex-col gap-2">
        <strong className="text-sm font-medium">Restored sessions ({touched.length})</strong>
        {touched.length ? (
          <div className="flex flex-col gap-2">
            {touched.map((item) => {
          ...
          <Empty>
            <EmptyHeader>
              <EmptyTitle>No sessions needed repair</EmptyTitle>
              <EmptyDescription>All Codex sessions for this workspace are already visible.</EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
      </div>
      ...
      {report.skipped_rollout_files?.length ? (
        <div className="flex flex-col gap-2">
          <strong className="text-sm font-medium">Skipped rollouts ({report.skipped_rollout_files.length})</strong>
...
export function AgentActionResultPanel({
  ...
  return (
    <Empty>
      <EmptyHeader>
        <EmptyTitle>Action completed</EmptyTitle>
        <EmptyDescription>No structured result renderer is available for this action yet.</EmptyDescription>
      </EmptyHeader>
    </Empty>
  );
}
